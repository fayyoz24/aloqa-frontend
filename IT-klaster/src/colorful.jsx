...
/* student tier helpers */
const isBlue   = (s) => (s?.color||"").toLowerCase()==="blue";
const isYellow = (s) => (s?.color||"").toLowerCase()==="yellow";
const tierBorder= (s,hov) => isBlue(s) ? (hov?"#1A3FDB":"#C4CCFA") : isYellow(s) ? (hov?"#B85C00":"#F5C97A") : (hov?T.borderMd:T.border);
const tierGlow  = (s)     => isBlue(s) ? "0 0 0 3px rgba(26,63,219,0.12)" : isYellow(s) ? "0 0 0 3px rgba(184,92,0,0.10)" : "";
const tierAvatarBg   = (s) => isBlue(s) ? T.blueLight  : isYellow(s) ? "#FFF8E7"  : T.blueLight;
const tierAvatarRing = (s) => isBlue(s) ? T.blueMid    : isYellow(s) ? T.amberMid : T.blueMid;
const tierAvatarText = (s) => isBlue(s) ? T.blue       : isYellow(s) ? T.amber    : T.blue;
...
const LockIco    = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="11" width="18" height="11" rx="2" strokeLinecap="round"/><path d="M7 11V7a5 5 0 0110 0v4" strokeLinecap="round"/></svg>;
const EyeIco     = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>;
const EyeOffIco  = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19m-6.72-1.07a3 3 0 11-4.24-4.24M1 1l22 22" strokeLinecap="round"/></svg>;
...
function Avatar({ student, size = "md" }) {
  ...
  const bg   = tierAvatarBg(student);
  const ring = tierAvatarRing(student);
  const col  = tierAvatarText(student);
  return (
    <div style={{ width:cfg.w, height:cfg.h, borderRadius:"50%", background:bg, color:col, display:"flex", alignItems:"center", justifyContent:"center", fontSize:cfg.fs, fontWeight:600, flexShrink:0, overflow:"hidden", border:`${cfg.bw} solid ${ring}` }}>
...
function Badge({ color="blue", children, href, onClick }) {
  const pal = {
    ...
    yellow: {bg:"#FFF8E7",     text:T.amber,  border:T.amberMid},
  };
...
/* ─── LOGIN MODAL ─── */
function LoginModal({ student, onSuccess, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!username || !password) { setError("Iltimos, barcha maydonlarni to'ldiring."); return; }
    ...
    try {
      ...
      if (!res.ok) { setError("Login yoki parol noto'g'ri."); setLoading(false); return; }
      const data = await res.json();
      if (data.token || data.access || res.ok) {
        onSuccess();
      } else {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:1000, background:"rgba(0,0,0,0.35)", display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:T.surface, borderRadius:T.radius, padding:"2rem", width:"100%", maxWidth:380, boxShadow:"0 20px 60px rgba(0,0,0,0.18)", animation:"fadeUp 0.25s ease both" }}>
        {/* Header */}
        <div style={{ textAlign:"center", marginBottom:"1.75rem" }}>
          <div style={{ width:52, height:52, borderRadius:"50%", background:T.blueLight, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 1rem", color:T.blue }}>
            <LockIco/>
          </div>
          <div style={{ fontSize:17, fontWeight:600, letterSpacing:"-0.02em", color:T.text, marginBottom:5 }}>Kirish talab etiladi</div>
          <div style={{ fontSize:13, color:T.text2, lineHeight:1.5 }}>
            <strong style={{ color:T.text }}>{student.first_name} {student.last_name}</strong> profilini ko'rish uchun tizimga kiring
          </div>
        </div>

        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"0.875rem" }}>
          <div>
            <label style={{ fontSize:12, fontWeight:500, color:T.text2, display:"block", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.05em" }}>Foydalanuvchi nomi</label>
            <input
              type="text" value={username} onChange={e=>setUsername(e.target.value)} placeholder="username"
              style={{ width:"100%", padding:"10px 14px", borderRadius:T.radiusSm, border:`1px solid ${T.borderMd}`, fontSize:14, color:T.text, background:T.surface, fontFamily:"inherit", outline:"none", transition:"border-color 0.15s, box-shadow 0.15s" }}
          ...
          <div>
            <label style={{ fontSize:12, fontWeight:500, color:T.text2, display:"block", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.05em" }}>Parol</label>
            <div style={{ position:"relative" }}>
              <input
                type={showPw?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••"
                style={{ width:"100%", padding:"10px 40px 10px 14px", borderRadius:T.radiusSm, border:`1px solid ${T.borderMd}`, fontSize:14, color:T.text, background:T.surface, fontFamily:"inherit", outline:"none", transition:"border-color 0.15s, box-shadow 0.15s" }}
                onFocus={e=>{ e.target.style.borderColor=T.blue; e.target.style.boxShadow=`0 0 0 3px rgba(26,63,219,0.1)`; }}
                onBlur={e=>{ e.target.style.borderColor=T.borderMd; e.target.style.boxShadow="none"; }}
              />
              <button type="button" onClick={()=>setShowPw(p=>!p)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:T.text3, display:"flex", padding:0 }}>
                {showPw ? <EyeOffIco/> : <EyeIco/>}
              </button>
            </div>
          </div>

          {error && <div style={{ fontSize:13, color:T.red, background:T.redLight, border:`1px solid rgba(192,39,26,0.2)`, borderRadius:T.radiusSm, padding:"8px 12px" }}>{error}</div>}

          <button type="submit" disabled={loading}
            style={{ width:"100%", padding:"11px", borderRadius:T.radiusSm, border:"none", background:T.blue, color:"#fff", fontSize:14, fontWeight:500, cursor:loading?"not-allowed":"pointer", opacity:loading?0.7:1, fontFamily:"inherit", marginTop:4, transition:"opacity 0.15s" }}>
            {loading ? "Kirilmoqda..." : "Kirish"}
          </button>
        </form>

        <button onClick={onClose} style={{ width:"100%", marginTop:10, padding:"8px", borderRadius:T.radiusSm, border:`1px solid ${T.border}`, background:"transparent", color:T.text2, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
          Bekor qilish
        </button>
      </div>
    </div>
    ...
    <div style={{ animation:"fadeUp 0.4s ease both" }}>
      ...
      <div style={{ display:"flex", justifyContent:"center", gap:"1.5rem", marginTop:"2rem", flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:T.text2 }}>
          <div style={{ width:14, height:14, borderRadius:"50%", background:T.blueLight, border:`2px solid ${T.blue}` }}/>
          Ko'k — Yuqori ball ✦
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:T.text2 }}>
          <div style={{ width:14, height:14, borderRadius:"50%", background:"#FFF8E7", border:`2px solid ${T.amberMid}` }}/>
          Sariq — Standart ball
        </div>
      </div>
    </div>
    ...
    <div style={{ animation:"fadeUp 0.4s ease both" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem" }}>
        {secs.map(({ key, icon, bg, color, title, desc }) => (
          <SecCard key={key} icon={icon} bg={bg} color={color} title={title} desc={desc} onClick={()=>onNav(key)}/>
        ))}
      </div>
    </div>
  );
...
function StudentsView({ stage, onNav, onStudent }) {
  ...
  const [loginTarget, setLoginTarget] = useState(null); // student pending login
  ...
  const handleCardClick = (s) => setLoginTarget(s);
  const handleLoginSuccess = () => { onStudent(loginTarget); setLoginTarget(null); };
  ...
  return (
    <div style={{ animation:"fadeUp 0.4s ease both" }}>
      ...
      <div style={{ display:"flex", gap:"1rem", flexWrap:"wrap", marginBottom:"1.25rem" }}>
        <div style={{ display:"flex", alignItems:"center", gap:7, fontSize:12, color:T.text2, background:T.blueLight, border:`1px solid ${T.blueMid}`, borderRadius:100, padding:"4px 12px" }}>
          <div style={{ width:10, height:10, borderRadius:"50%", background:T.blue }}/> Ko'k — Yuqori ball ✦
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:7, fontSize:12, color:T.amber, background:"#FFF8E7", border:`1px solid ${T.amberMid}`, borderRadius:100, padding:"4px 12px" }}>
          <div style={{ width:10, height:10, borderRadius:"50%", background:T.amber }}/> Sariq — Standart ball
        </div>
      </div>
      ...
      {loading ? <Spinner/> : error ? <ErrorState msg={error}/> : filtered.length===0 ? <EmptyState msg="Talaba topilmadi"/> :
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))", gap:"1rem" }}>
          {filtered.map(s => <StudentCard key={s.id} student={s} onClick={()=>handleCardClick(s)}/>)}
        </div>
      }
...
function StudentCard({ student:s, onClick }) {
  ...
  const blue   = isBlue(s);
  const yellow = isYellow(s);

  return (
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{
        background: blue ? `linear-gradient(145deg, #EEF1FD 0%, ${T.surface} 60%)` : yellow ? `linear-gradient(145deg, #FFF8E7 0%, ${T.surface} 60%)` : T.surface,
        border: `1.5px solid ${tierBorder(s,hov)}`,
        borderRadius: T.radius,
        padding: "1.25rem",
        cursor: "pointer",
        transition: "transform 0.18s, box-shadow 0.18s, border-color 0.18s",
        transform: hov ? "translateY(-3px)" : "none",
        boxShadow: hov ? (tierGlow(s) ? `${T.shadowMd}, ${tierGlow(s)}` : T.shadowMd) : (blue||yellow ? `${T.shadowSm}, ${tierGlow(s)}` : "none"),
        display: "flex",
      ...
      {/* Tier stripe at top */}
      {blue   && <div style={{ position:"absolute", top:0, left:0, right:0, height:3, background:T.blue, borderRadius:"12px 12px 0 0" }}/>}
      {yellow && <div style={{ position:"absolute", top:0, left:0, right:0, height:3, background:T.amberMid, borderRadius:"12px 12px 0 0" }}/>}

      <div style={{ display:"flex", alignItems:"center", gap:12, marginTop: (blue||yellow) ? 4 : 0 }}>
        ...
        {/* Tier badge */}
        {blue   && <span style={{ fontSize:11, fontWeight:600, color:T.blue,  background:T.blueLight,  border:`1px solid ${T.blueMid}`,  borderRadius:100, padding:"3px 9px", whiteSpace:"nowrap", letterSpacing:"0.02em" }}>✦ Yuqori</span>}
        {yellow && <span style={{ fontSize:11, fontWeight:600, color:T.amber, background:"#FFF8E7",    border:`1px solid ${T.amberMid}`, borderRadius:100, padding:"3px 9px", whiteSpace:"nowrap" }}>Standart</span>}
      </div>
  ...
  return (
    <div style={{ animation:"fadeUp 0.4s ease both" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"300px 1fr", gap:"1.25rem", alignItems:"start" }}>
        {/* Sidebar */}
        <div style={{ background:T.surface, border:`1.5px solid ${tierBorder(s,false)}`, borderRadius:T.radius, padding:"1.5rem", position:"sticky", top:"calc(60px + 1rem)", overflow:"hidden" }}>
          {isBlue(s)   && <div style={{ position:"absolute", top:0, left:0, right:0, height:4, background:T.blue }}/>}
          {isYellow(s) && <div style={{ position:"absolute", top:0, left:0, right:0, height:4, background:T.amberMid }}/>}
          <div style={{ textAlign:"center", marginBottom:"1.25rem", marginTop:(isBlue(s)||isYellow(s))?8:0 }}><Avatar student={s} size="lg"/></div>
          ...
          {s.middle_name && <div style={{ fontSize:13, color:T.text3, textAlign:"center", marginBottom:8 }}>{s.middle_name}</div>}
          <div style={{ display:"flex", justifyContent:"center", marginBottom:"1.25rem" }}>
            {isBlue(s)   && <Badge color="blue">✦ Yuqori ball</Badge>}
            {isYellow(s) && <Badge color="amber">Standart ball</Badge>}
          </div>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13, minWidth:600 }}>
              ...
              <tbody>
                {rows.map((r,i) => {
                  ...
                  const rowBlue   = isBlue(r.s);
                  const rowYellow = isYellow(r.s);
                  return (
                    <tr key={r.s.id}
                      style={{ borderBottom:`1px solid ${T.border}`, background: rowBlue ? "#F7F9FF" : rowYellow ? "#FFFBF2" : "transparent", transition:"background 0.1s" }}
                      onMouseEnter={e=>e.currentTarget.style.background=rowBlue?"#EEF1FD":rowYellow?"#FFF8E7":T.surface2}
                      onMouseLeave={e=>e.currentTarget.style.background=rowBlue?"#F7F9FF":rowYellow?"#FFFBF2":"transparent"}>
                      ...
                      <td style={{ padding:"10px 14px" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:9 }}>
                          ...
                          {rowBlue   && <span style={{ fontSize:10, fontWeight:600, color:T.blue,  background:T.blueLight,  border:`1px solid ${T.blueMid}`,  borderRadius:100, padding:"2px 7px" }}>✦</span>}
                          {rowYellow && <span style={{ fontSize:10, fontWeight:600, color:T.amber, background:"#FFF8E7",    border:`1px solid ${T.amberMid}`, borderRadius:100, padding:"2px 7px" }}>●</span>}
                        </div>
                      </td>
